...
class NetworkDeploymentEnv(gym.Env):
    def __init__(self):
        ...
        self.fixed_donor_indices = self.define_fixed_donors()
        self.current_step = 0
    ...
    def define_fixed_donors(self):
        # Define fixed positions for donor nodes, ensure they are within bounds
        return [self.n_potential_nodes_per_row * i + i for i in range(self.numberofdonor)]
    ...
    def deploy_node(self, node_index):
        ...
        if self.state["D"][x, y] == 1:
            ...
            if not connected:
                print(f"Deployed node {[x* self.node_spacing, y* self.node_spacing]} could not find a node to connect. High penalty applied.")
                self.state["D"][x, y] = 0
                rewards = self.calculate_reward()
                return rewards
    ...
    def calculate_distance_2d(self, x1, y1, x2, y2):
        ...
        dx = (x2 - x1) * self.node_spacing
        dy = (y2 - y1) * self.node_spacing
    ...
    def calculate_addcoverage(self):
        ...
        for i in range(self.n_potential_nodes):
            ...
            if self.state["D"][x, y] == 1:
                node_x = y * self.node_spacing
                node_y = x * self.node_spacing
