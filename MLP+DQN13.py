...
class Agent:
    ...
    def experience_replay(self, batch_size):
        ...
        loss.backward()
        self.optimizer.step()
            # Debugging: Print learning rate and scheduler step information
        current_lr = self.get_current_lr()
        # print(f"Before scheduler step - Episode: {self.total_steps // batch_size}, Loss: {loss.item():.4f}, Learning Rate: {current_lr:.6f}")
        self.scheduler.step(loss)
            # Print updated learning rate after scheduler step
        updated_lr = self.get_current_lr()
        # print(f"After scheduler step - Episode: {self.total_steps // batch_size}, Loss: {loss.item():.4f}, Learning Rate: {updated_lr:.6f}")
...
def train(env, agent, episodes, batch_size=512, target_update=128, scale=300,save_model_interval=30000):
    episode_rewards = []
    episode_losses = []
    data = {
        "Episode": [],
    ...
    for episode in range(episodes):
        ...
        # Save final model
    torch.save(agent.model.state_dict(), 'model_final1.pth')
